<?php
    ...
    if(!isset($get_id))
    {
        if($username)
        {
            go_to_page("./user.php?id=".$username,"");
            exit();
        }
        go_to_404();
        exit;
    ...
    $query = "select * from user where u_id = '".$get_id."' and u_status =1";
...
<head>
	<meta charset="UTF-8">
	<title><?php echo $info['u_nickname'] ?>的照片收集 - 美图册</title>
...
<div class="self-wrapper">
    ...
    ?>
        ...
        <div class="self-detail">
            ...
            <p id="self-describe"><?php if(isset($info['u_des'])) echo $info['u_des']; else echo "主人比较懒~还未填写任何介绍哦~"; ?></p>
            <p id="self-follow"><?php echo $like_num['num'];?>次被喜欢&nbsp;&nbsp;&nbsp;&nbsp;<?php echo $photo_num['num'];?>个收集&nbsp;&nbsp;&nbsp;&nbsp;<?php echo $album_num['num'];?>个相册&nbsp;&nbsp;&nbsp;&nbsp;<span class="icon-location2"></span><?php if(empty($info['u_location']))echo "未知的星球";else echo $info['u_location'];?></p>
        </div>
    </div>
    ...
    <div class="self-menu">
    <ul>
    	<li><a href="user.php?id=<?php echo $get_id ?>" class="active"><span class="icon-image"></span>图片（<?php echo $photo_num['num'];?>）</a></li>
        <li><a href="user_album.php?id=<?php echo $get_id ?>"><span class="icon-images"></span>图册（<?php echo $album_num['num'];?>）</a></li>
       </ul>
    </div>

    <?php
        $query = "select * from photo where p_user_id='".$get_id."' order by p_time desc";
        $user_photo = mysqli_query($conn,$query);
    ?>

    <div class="result">
         <?php 
            display_photo_box($conn,$user_photo);
        ?>
        <div class="clear"></div>    	
    </div>   
</div>
...
<div id="build-album" class="build-album tip-buble">
    <form class='new-album-box tip-buble-box center' action="user/build_album.php" method="post">
